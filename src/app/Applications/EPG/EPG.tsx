...
function roundDownToNearestMinutes(date: Date, roundMinutes: number) {
...
const EPG: React.FC<ClassicyEPGProps> = ({
    ...
    const [gridStartTime, setGridStartTime] = useState(roundDownToNearestMinutes(gridStart, gridTimeWidth))
        ...
        for (let i = 1; i <= gridWidth / minutesPerGrid; i += gridTimeWidth / minutesPerGrid) {
            headers.push(
                <div className={epgStyles.epgHeaderTime} style={{ gridColumn: `${i + 1} / span ${minutesPerGrid}` }}>
                    {new Date(gridStartTime.getTime() + (i - 1) * minutesPerGrid * 60000).toLocaleTimeString([], {
                        hour: 'numeric',
                        minute: '2-digit',
                    })}
                </div>
        ...
        <>
            <ClassicyApp id={appId} name={appName} icon={appIcon} defaultWindow={'demo'}>
                <ClassicyWindow
                    ...
                    appMenu={appMenu}
                >
                    <div style={{ height: '100%', width: '100%', backgroundColor: 'var(--color-system-02)' }}>
                        <div
                        ...
                        <div className={epgStyles.epgControlsHolder}>
